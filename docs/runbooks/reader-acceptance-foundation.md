...
## Pending durable evidence — Chrome-only per #104#issuecomment-5353353146

Later tickets #98–#103 must fill the final acceptance report with curated screenshots/contact sheets, complete wide/320/light/dark/reduced-motion/zoom/text-spacing results for current stable Chrome (Chromium automated + Chrome 151.0.7922.169 human evidence: visual 32-image matrix PASS, Stage 1 interaction PASS, Stage 2 320px touch + 200/400 zoom PASS, Stage 3 text-spacing + reduced-motion + no-JS PASS), contrast samples covering all three callout variants (fact/note/warning) in light/dark, Lighthouse results, complete Reader and Studio regression evidence, material deviations/human decisions, and final explicit human fidelity approval (contrast + M13 remain BLOCKED_PENDING_HUMAN). Firefox, native Safari, Playwright WebKit proxy, and Orca + Firefox/real-AT journeys are no longer gates per Chrome-only amendment (spec reduction, not fabricated PASS) — Firefox Dev 155.0b1 supplemental PASS noted separately, Orca installed then removed per human request — report must state reduced cross-browser and real-AT coverage as residual limitation. None of those pending cells weakens the fixed invariants above.

The deterministic rich-article fixture now exposes all three callout variants (fact, note, warning) for light/dark manual contrast sampling; production CSS and asset budgets unchanged (test-only fixture coverage). If fixture adds variants, regenerate Chromium screenshots (article-rich 4 PNGs delta) and re-verify reflow.
