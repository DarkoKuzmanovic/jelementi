...
- `lighthouse.json` — concise mobile Lighthouse evidence for `/` (Accessibility 100, Best Practices 100, SEO 50 (expected due to global noindex), Performance 100, version + URL + commit). Raw `*.json`/`*.html` at `/tmp/lighthouse-T104-*` are CI artifacts, not committed. Performance is rerun if noisy, not waived.
